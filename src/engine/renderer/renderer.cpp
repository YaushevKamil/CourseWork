...
namespace Engine {
    ...
    void Renderer::Init(GLADloadproc loadProcFunc) {
        ...
        Engine::Texture *hdrMap = Engine::Resources::LoadHDR("sky env", "textures/backgrounds/alley.hdr");

        Engine::PBRCapture *envBridge = m_PBR->ProcessEquirectangular(hdrMap);
    ...
    void Renderer::PushRender(SceneNode* node) {
        ...
        while (!nodeStack.empty()) {
            SceneNode* t_node = nodeStack.top();
            nodeStack.pop();
            // only push render command if the child isn't a container t_node.
            if (t_node->m_Mesh) {
                math::vec3 boxMinWorld = t_node->GetWorldPosition() + (t_node->GetWorldScale() * t_node->BoxMin);
                math::vec3 boxMaxWorld = t_node->GetWorldPosition() + (t_node->GetWorldScale() * t_node->BoxMax);
                m_CommandBuffer->Push(t_node->m_Mesh, t_node->m_Material, t_node->GetTransform(), t_node->GetPrevTransform(), boxMinWorld, boxMaxWorld, target);
            }
            for (unsigned int i = 0; i < t_node->GetChildCount(); ++i) {
                nodeStack.push(t_node->GetChildByIndex(i));
            }
        }
    }
    ...
    void Renderer::RenderPushedCommands() {
        ...
        if (Shadows) {
            ...
            for (int i = 0; i < m_DirectionalLights.size(); ++i) {
                ...
                if (light->CastShadows) {
                    ...
                    math::vector<3, float> direction = -light->Direction * 10.0f;
                    math::vector<3, float> target = math::vec3(0.0);
                    math::mat4 lightView = math::lookAt(direction, target, math::vec3::UP);
        ...
        for (auto it = m_PointLights.begin(); it != m_PointLights.end(); ++it) {
            if ((*it)->RenderMesh) {
                ...
                command.c_Material = m_MaterialLibrary->debugLightMaterial;
                command.c_Mesh = m_DebugLightMesh;
                math::mat4 model;
                math::translate(model, (*it)->Position);
                math::vec3 scale(0.25f);
                math::scale(model, scale);
                command.c_Transform = model;
        ...
        for (unsigned int i = 0; i < postProcessingCommands.size(); ++i) {
            ...
            Blit(even ? m_CustomTarget->GetColorTexture(0) : m_PostProcessTarget1->GetColorTexture(0),
                 ...
                 postProcessingCommands[i].c_Material);
        }
    ...
    void Renderer::Blit(Texture*      src,
        ...
        if (dst) {
            ...
            if (dst->HasDepthAndStencil)
                ...
                glClear(GL_COLOR_BUFFER_BIT);
        } else {  // else we bind to the default framebuffer
    ...
    void Renderer::BakeProbes(SceneNode* scene) {
        ...
        while (!sceneStack.empty()) {
            ...
            sceneStack.pop();
            if (node->m_Mesh) {
                auto samplerUniforms = *(node->m_Material->GetSamplerUniforms());
                if (samplerUniforms.find("TexAlbedo") != samplerUniforms.end()) {
                    ...
                    commandBuffer.Push(node->m_Mesh, materials[materials.size() - 1], node->GetTransform());
                    ...
                    materials[materials.size() - 1]->DepthCompare = node->m_Material->DepthCompare;
                    commandBuffer.Push(node->m_Mesh, materials[materials.size() - 1], node->GetTransform());
                }
            }
            for (unsigned int i = 0; i < node->GetChildCount(); ++i)
    ...
    void Renderer::renderCustomCommand(RenderCommand* command, Camera* customCamera, bool updateGLSettings) {
        Material *material = command->c_Material;
        Mesh     *mesh     = command->c_Mesh;
        ...
        material->GetShader()->SetMatrix("model", command->c_Transform);
        material->GetShader()->SetMatrix("prevModel", command->c_PrevTransform);
    ...
    void Renderer::renderToCubemap(SceneNode* scene,
        ...
        CommandBuffer commandBuffer(this);
        commandBuffer.Push(scene->m_Mesh, scene->m_Material, scene->GetTransform());
        ...
        while (!childStack.empty()) {
            ...
            childStack.pop();
            commandBuffer.Push(child->m_Mesh, child->m_Material, child->GetTransform());
    ...
    void Renderer::renderToCubemap(std::vector<RenderCommand>& renderCommands, TextureCube* target, math::vec3 position, unsigned int mipLevel) {
        ...
        for (unsigned int i = 0; i < 6; ++i) {
            ...
            for (unsigned int i = 0; i < renderCommands.size(); ++i) {
                ...
                assert(renderCommands[i].c_Material->Type == MATERIAL_CUSTOM);
    ...
    void Renderer::updateGlobalUBOs() {
        ...
        for (unsigned int i = 0; i < m_DirectionalLights.size() && i < 4; ++i) {
    ...
    void Renderer::renderDeferredAmbient() {
        ...
        if (IrradianceGI && irradianceProbes.size() > 0) {
            ...
            m_GLCache.SetCullFace(GL_BACK);
        } else { // otherwise do a full-screen ambient pass
    ...
    void Renderer::renderShadowCastCommand(RenderCommand* command, const math::mat4& projection, const math::mat4& view) {
        ...
        shadowShader->SetMatrix("model", command->c_Transform);

        renderMesh(command->c_Mesh, shadowShader);
    }
}